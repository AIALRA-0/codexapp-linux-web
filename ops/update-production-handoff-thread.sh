...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "production handoff update must run as root" >&2
  exit 77
fi
...
if [[ ! "$user_key" =~ ^[0-9a-f]{64}$ ]] ||
  ...
  [[ -z "$thread_name" ]] || [[ ! "$expected_commit" =~ ^[0-9a-f]{40}$ ]]; then
  echo "invalid production handoff update arguments" >&2
  exit 64
fi
for required in "$environment_file" "$auth_database" \
  "$application_root/scripts/update-handoff-thread.mjs"; do
  if [[ ! -f "$required" || -L "$required" ]]; then
    echo "required production handoff update input is missing or symbolic: $required" >&2
    exit 66
  fi
done
...
if ! jq -e '.ok == true and .active == false' <<<"$background_snapshot" >/dev/null; then
  echo "A has active work; handoff update refused" >&2
  exit 75
fi
...
HANDOFF_EXPECTED_COMMIT="$expected_commit" \
  node "$application_root/scripts/update-handoff-thread.mjs"
